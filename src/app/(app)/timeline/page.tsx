import Link from "next/link";
import {
  addTimelineEvent,
  deleteTimelineEvent,
  trackTimelineEvent,
} from "@/app/actions";
import { createClient } from "@/lib/supabase/server";
import type { TimelineEvent } from "@/lib/types";
import {
  Button,
  Card,
  Field,
  Input,
  PageHeader,
  Textarea,
  cn,
  formatDate,
} from "@/lib/ui";

function monthKey(dateStr: string | null): string {
  if (!dateStr) return "Undated";
  const d = new Date(`${dateStr}T00:00:00`);
  if (Number.isNaN(d.getTime())) return "Undated";
  return d.toLocaleDateString("en-US", { month: "long", year: "numeric" });
}

function windowState(e: TimelineEvent): "past" | "open" | "upcoming" {
  const today = new Date().toISOString().slice(0, 10);
  if (e.ends_on && e.ends_on < today) return "past";
  if (e.starts_on && e.starts_on <= today) return "open";
  return "upcoming";
}

export default async function TimelinePage() {
  const supabase = await createClient();
  const { data } = await supabase
    .from("timeline_events")
    .select("*")
    .order("starts_on", { ascending: true, nullsFirst: false })
    .order("company", { ascending: true });
  const events = (data ?? []) as TimelineEvent[];

  // Group into ordered month buckets; undated events go last.
  const groups: { label: string; events: TimelineEvent[] }[] = [];
  for (const e of events) {
    const label = monthKey(e.starts_on);
    const last = groups[groups.length - 1];
    if (last && last.label === label) last.events.push(e);
    else groups.push({ label, events: [e] });
  }
  const undatedIdx = groups.findIndex((g) => g.label === "Undated");
  if (undatedIdx >= 0) groups.push(...groups.splice(undatedIdx, 1));

  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <PageHeader
        title="Application timeline"
        description="When each company's application window opens — so you apply in week one, not week six. Ask Claude Code or Codex to research companies and import their timelines via MCP; events land here automatically."
      />

      {events.length === 0 ? (
        <Card className="border-dashed p-12 text-center shadow-none">
          <h2 className="text-lg font-semibold text-stone-900">
            No timeline yet
          </h2>
          <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-stone-500">
            In Claude Code, try:{" "}
            <code className="rounded-md bg-stone-100 px-1.5 py-0.5 font-mono text-xs">
              Research when big pharma ML internships open and build my
              application timeline in Career Copilot
            </code>
          </p>
        </Card>
      ) : (
        <div className="relative space-y-10 before:absolute before:top-1.5 before:bottom-1.5 before:left-[7px] before:w-px before:bg-stone-200">
          {groups.map((group) => {
            const groupOpen = group.events.some(
              (e) => windowState(e) === "open"
            );
            const groupPast = group.events.every(
              (e) => windowState(e) === "past"
            );
            return (
              <section key={group.label} className="relative pl-9">
                <span
                  className={cn(
                    "absolute top-0.5 left-0 size-[15px] rounded-full border-[3px] bg-white",
                    groupOpen
                      ? "border-indigo-500"
                      : groupPast
                        ? "border-stone-200"
                        : "border-stone-300"
                  )}
                />
                <h2
                  className={cn(
                    "text-xs font-semibold tracking-wider uppercase",
                    groupPast ? "text-stone-400" : "text-stone-600"
                  )}
                >
                  {group.label}
                </h2>
                <div className="mt-3 space-y-3">
                  {group.events.map((e) => {
                    const state = windowState(e);
                    return (
                      <Card
                        key={e.id}
                        className={cn("p-4", state === "past" && "opacity-55")}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-baseline gap-x-2">
                              <span className="font-medium text-stone-900">
                                {e.company}
                              </span>
                              {e.program && (
                                <span className="text-sm text-stone-500">
                                  {e.program}
                                </span>
                              )}
                            </div>
                            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs tabular-nums">
                              {e.window_label && (
                                <span className="font-medium text-stone-600">
                                  {e.window_label}
                                </span>
                              )}
                              {e.ends_on && state !== "past" && (
                                <span className="text-stone-400">
                                  closes {formatDate(e.ends_on)}
                                </span>
                              )}
                              {e.url && (
                                <a
                                  href={e.url}
                                  target="_blank"
                                  rel="noreferrer"
                                  className="font-medium text-indigo-600 underline underline-offset-2 hover:text-indigo-700"
                                >
                                  Program page ↗
                                </a>
                              )}
                            </div>
                          </div>
                          {state === "open" && (
                            <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-xs font-medium text-emerald-700">
                              <span className="size-1.5 rounded-full bg-emerald-500" />
                              Open now
                            </span>
                          )}
                        </div>
                        {e.notes && (
                          <p className="mt-2 text-sm leading-relaxed whitespace-pre-wrap text-stone-600">
                            {e.notes}
                          </p>
                        )}
                        <div className="mt-3 flex items-center gap-2">
                          {e.job_id ? (
                            <Link
                              href={`/jobs/${e.job_id}`}
                              className="text-xs font-medium text-indigo-600 hover:text-indigo-700"
                            >
                              View tracked job →
                            </Link>
                          ) : (
                            <form action={trackTimelineEvent.bind(null, e.id)}>
                              <Button variant="secondary" size="sm">
                                Track as job
                              </Button>
                            </form>
                          )}
                          <form
                            action={deleteTimelineEvent.bind(null, e.id)}
                            className="ml-auto"
                          >
                            <Button variant="danger" size="sm">
                              Delete
                            </Button>
                          </form>
                        </div>
                      </Card>
                    );
                  })}
                </div>
              </section>
            );
          })}
        </div>
      )}

      <Card>
        <details>
          <summary className="flex cursor-pointer items-center justify-between gap-3 rounded-xl px-4 py-3 text-sm font-medium text-stone-800 transition-colors hover:bg-stone-50">
            <span>+ Add event manually</span>
            <span className="text-xs text-stone-300">▾</span>
          </summary>
          <form
            action={addTimelineEvent}
            className="space-y-4 border-t border-stone-100 p-4"
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Company" hint="required">
                <Input name="company" required placeholder="Genentech" />
              </Field>
              <Field label="Program / role">
                <Input name="program" placeholder="Summer Internship Program" />
              </Field>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
              <Field label="Window label">
                <Input
                  name="window_label"
                  placeholder="Nov 2026 – Mar 2027 (rolling)"
                />
              </Field>
              <Field label="Window opens">
                <Input name="starts_on" type="date" />
              </Field>
              <Field label="Window closes">
                <Input name="ends_on" type="date" />
              </Field>
            </div>
            <Field label="Program URL">
              <Input name="url" type="url" placeholder="https://..." />
            </Field>
            <Field label="Notes" hint="eligibility, tips, sources">
              <Textarea name="notes" rows={3} />
            </Field>
            <Button>Add event</Button>
          </form>
        </details>
      </Card>
    </div>
  );
}
