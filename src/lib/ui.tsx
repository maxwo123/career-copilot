...
const BUTTON_VARIANTS: Record<ButtonVariant, string> = {
  ...
  secondary:
    "border border-stone-300 bg-white font-medium text-stone-700 shadow-xs hover:border-stone-400 hover:text-stone-900",
  ghost: "font-medium text-stone-500 hover:bg-stone-100 hover:text-stone-800",
  danger: "font-medium text-stone-400 hover:bg-red-50 hover:text-red-600",
};
...
const controlCls =
  "w-full rounded-lg border border-stone-300 bg-white px-3 text-sm text-stone-900 shadow-xs transition-colors placeholder:text-stone-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/15 focus:outline-none";
...
export function Field({
  ...
  return (
    <label className={cn("block", className)}>
      <span className="mb-1.5 flex items-baseline justify-between gap-2 text-[13px] font-medium text-stone-700">
        <span>{label}</span>
...
export function Card({
  ...
  return (
    <div className={cn("rounded-xl border border-stone-200 bg-white shadow-xs", className)}>
      {children}
    </div>
  );
}
...
export function SectionTitle({
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      ...
      {typeof count === "number" && (
        <span className="rounded-full bg-stone-200/70 px-1.5 py-px text-[11px] font-medium text-stone-500 tabular-nums">
          {count}
        </span>
      )}
...
export function Badge({
  ...
  return (
    <span
      className={cn(
        "inline-flex shrink-0 items-center rounded-full border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-[11px] font-medium text-indigo-700",
        className
      )}
    >
      {children}
